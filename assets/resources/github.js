	...
	function render(target, repos, options){
			...
			for(i = 0; i < repos.length; i++) {
				if (repos[i].name != exA && repos[i].name != exB) {
					fragment += '<li class="' +options.itemClass+ '"><a title="' +escapeHtml(repos[i].description||'')+ '" href="' +repos[i].html_url+ '">' +repos[i].name+ '</a>';
					if (options.descriptions!==false) {
						fragment +='<p>'+escapeHtml(repos[i].description||'')+'</p>';
					}
					fragment += '</li>';
				}
			}
